    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center gap-8 bg-black">
      {language === "english" ? (
        <AppleHelloEnglishEffect
          ...
          onAnimationComplete={() => setTimeout(() => setLanguage("vietnamese"), 1000)}
        />
      ) : (
        <AppleHelloVietnameseEffect
          className="text-white h-24"
          onAnimationComplete={() => setTimeout(() => setLanguage("english"), 1000)}
        />
      )}
      <p className="text-white/50 text-sm">Animation cycles between English and Vietnamese</p>
    </div>
  )
}
