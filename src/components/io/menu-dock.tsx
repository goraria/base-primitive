      ...
      {(!showLabels || orientation === "vertical") && (
        <div
          className={cn(
            ...
            orientation === "vertical" ? "left-1 w-1 h-6" : "bottom-0.5 h-0.5 w-6",
          )}
          style={{
...
// Demo
import { Film, Image, Mic, Music, Radio } from "lucide-react"

export function Demo() {
  const mediaItems: MenuDockItem[] = [
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center pb-8">
      <MenuDock items={mediaItems} variant="large" />
    </div>
  )
}
